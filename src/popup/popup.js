...
/** Search-engine ids set to open bookmarks in the same tab (mirrors the SW). */
let sameTabEngines = new Set();
let searchEngineRendered = false;
let folderSeq = 0;
...
function onSearchEngineToggle() {
  ...
  if (open && !searchEngineRendered) {
    renderSearchEngines();
    searchEngineRendered = true;
  }
}
